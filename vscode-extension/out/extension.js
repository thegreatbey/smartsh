...
const smartsh_1 = require("smartsh");
function activate(context) {
    ...
    let translateCommandDisposable = vscode.commands.registerCommand('smartsh.translateCommand', async () => {
        ...
        if (input) {
            const shell = (0, smartsh_1.detectShell)();
            const translated = (0, smartsh_1.translateCommand)(input, shell);
    ...
    let translateSelectionDisposable = vscode.commands.registerCommand('smartsh.translateSelection', async () => {
        ...
        const shell = (0, smartsh_1.detectShell)();
        const translated = (0, smartsh_1.translateCommand)(text, shell);
    ...
    statusBarItem.text = `$(terminal) ${(0, smartsh_1.detectShell)().type}`;
    ...
    const hoverProvider = vscode.languages.registerHoverProvider(['shellscript', 'bash'], {
        provideHover(document, position, token) {
            ...
            if (unixCommands.includes(word)) {
                const shell = (0, smartsh_1.detectShell)();
                const translated = (0, smartsh_1.translateCommand)(word, shell);
    ...
    context.subscriptions.push(translateCommandDisposable, translateSelectionDisposable, statusBarItem, hoverProvider, completionProvider);
}
function deactivate() {
